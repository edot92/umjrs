...
var App = (function () {
  var t,
    ...
    p = function p () {
      $('body').css('direction') === 'rtl' && (e = !0), o = !!navigator.userAgent.match(/MSIE 8.0/), a = !!navigator.userAgent.match(/MSIE 9.0/), i = !!navigator.userAgent.match(/MSIE 10.0/), i && $('html').addClass('ie10'), (i || a || o) && $('html').addClass('ie')
    },
    h = function h () {
    ...
    f = function f () {
      $('body').on('click', '.portlet > .portlet-title > .tools > a.remove', function (t) {
        ...
        t.preventDefault(); var e = $(this).closest('.portlet').children('.portlet-body'),
          ...
          a = $(this).attr('data-error-display'); o ? (App.blockUI({ target: e, animate: !0, overlayColor: 'none' }), $.ajax({ type: 'GET',
            cache: !1,
            url: o,
            dataType: 'html',
            ...
            error: function error (t, o, i) {
              App.unblockUI(e); var n = 'Error on reloading the content. Please check your connection and try again.'; a == 'toastr' && toastr ? toastr.error(n) : a == 'notific8' && $.notific8 ? ($.notific8('zindex', 11500), $.notific8(n, { theme: 'ruby', life: 3e3 })) : alert(n)
    ...
    A = function A () {
      typeof autosize === 'function' && autosize(document.querySelectorAll('textarea.autosizeme'))
    },
    S = function S () {
    ...
    E = function E () {
      $('[data-auto-height]').each(function () {
        var t = $(this),
          ...
          i = parseInt(t.attr('data-offset') ? t.attr('data-offset') : 0); e.each(function () {
            $(this).attr('data-height') == 'height' ? $(this).css('height', '') : $(this).css('min-height', ''); var t = a == 'base-height' ? $(this).outerHeight() : $(this).outerHeight(!0); t > o && (o = t)
          }), o += i, e.each(function () {
            $(this).attr('data-height') == 'height' ? $(this).css('height', o) : $(this).css('min-height', o)
      ...
      scrollTo: function scrollTo (t, e) {
        var o = t && t.size() > 0 ? t.offset().top : 0; t && ($('body').hasClass('page-header-fixed') ? o -= $('.page-header').height() : $('body').hasClass('page-header-top-fixed') ? o -= $('.page-header-top').height() : $('body').hasClass('page-header-menu-fixed') && (o -= $('.page-header-menu').height()), o += e || -1 * t.height()), $('html,body').animate({ scrollTop: o }, 'slow')
      },
      initSlimScroll: function initSlimScroll (t) {
        $().slimScroll && $(t).each(function () {
          if (!$(this).attr('data-initialized')) {
            var t; t = $(this).attr('data-height') ? $(this).attr('data-height') : $(this).css('height'), $(this).slimScroll({ allowPageScroll: !0, size: '7px', color: $(this).attr('data-handle-color') ? $(this).attr('data-handle-color') : '#bbb', wrapperClass: $(this).attr('data-wrapper-class') ? $(this).attr('data-wrapper-class') : 'slimScrollDiv', railColor: $(this).attr('data-rail-color') ? $(this).attr('data-rail-color') : '#eaeaea', position: e ? 'left' : 'right', height: t, alwaysVisible: $(this).attr('data-always-visible') == '1', railVisible: $(this).attr('data-rail-visible') == '1', disableFadeOut: !0 }), $(this).attr('data-initialized', '1')
          }
        })
      },
      ...
      blockUI: function blockUI (t) {
        t = $.extend(!0, {}, t); var e = ''; if (e = t.animate ? '<div class="loading-message ' + (t.boxed ? 'loading-message-boxed' : '') + '"><div class="block-spinner-bar"><div class="bounce1"></div><div class="bounce2"></div><div class="bounce3"></div></div></div>' : t.iconOnly ? '<div class="loading-message ' + (t.boxed ? 'loading-message-boxed' : '') + '"><img src="' + this.getGlobalImgPath() + 'loading-spinner-grey.gif" align=""></div>' : t.textOnly ? '<div class="loading-message ' + (t.boxed ? 'loading-message-boxed' : '') + '"><span>&nbsp;&nbsp;' + (t.message ? t.message : 'LOADING...') + '</span></div>' : '<div class="loading-message ' + (t.boxed ? 'loading-message-boxed' : '') + '"><img src="' + this.getGlobalImgPath() + 'loading-spinner-grey.gif" align=""><span>&nbsp;&nbsp;' + (t.message ? t.message : 'LOADING...') + '</span></div>', t.target) {
          var o = $(t.target); o.height() <= $(window).height() && (t.cenrerY = !0), o.block({ message: e, baseZ: t.zIndex ? t.zIndex : 1e3, centerY: void 0 !== t.cenrerY && t.cenrerY, css: { top: '10%', border: '0', padding: '0', backgroundColor: 'none' }, overlayCSS: { backgroundColor: t.overlayColor ? t.overlayColor : '#555', opacity: t.boxed ? 0.05 : 0.1, cursor: 'wait' }})
        } else $.blockUI({ message: e, baseZ: t.zIndex ? t.zIndex : 1e3, css: { border: '0', padding: '0', backgroundColor: 'none' }, overlayCSS: { backgroundColor: t.overlayColor ? t.overlayColor : '#555', opacity: t.boxed ? 0.05 : 0.1, cursor: 'wait' }})
      },
      ...
      alert: function alert (t) {
        t = $.extend(!0, { container: '', place: 'append', type: 'success', message: '', close: !0, reset: !0, focus: !0, closeInSeconds: 0, icon: '' }, t); var e = App.getUniqueID('App_alert'),
          o = '<div id="' + e + '" class="custom-alerts alert alert-' + t.type + ' fade in">' + (t.close ? '<button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button>' : '') + (t.icon !== '' ? '<i class="fa-lg fa fa-' + t.icon + '"></i>  ' : '') + t.message + '</div>'; return t.reset && $('.custom-alerts').remove(), t.container ? t.place == 'append' ? $(t.container).append(o) : $(t.container).prepend(o) : $('.page-fixed-main-content').size() === 1 ? $('.page-fixed-main-content').prepend(o) : ($('body').hasClass('page-container-bg-solid') || $('body').hasClass('page-content-white')) && $('.page-head').size() === 0 ? $('.page-title').after(o) : $('.page-bar').size() > 0 ? $('.page-bar').after(o) : $('.page-breadcrumb, .breadcrumbs').after(o), t.focus && App.scrollTo($('#' + e)), t.closeInSeconds > 0 && setTimeout(function () {
            $('#' + e).remove()
